"""CodeFlow CLI — multi-agent code intelligence pipeline."""
...
@click.group()
@click.version_option(version="0.1.0", prog_name="codeflow")
def cli():
    """CodeFlow — AI-powered multi-agent code analysis & refactoring pipeline.
...
@click.option("--dry-run", is_flag=True, help="Run scanner only, skip modification stages")
@click.option("--output", "-o", default="codeflow_report.json", help="Save full report to JSON file")
...
def scan(target):
    ...
    try:
        ...
        if result.success:
            pipeline.save_report("codeflow_scan.json")
    except ValueError as e:
